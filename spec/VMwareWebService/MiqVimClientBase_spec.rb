...
describe MiqVimClientBase do
  ...
  context "#sdk_uri" do
    it "IPv4" do
      expect(described_class.new("127.0.0.1", nil, nil, nil).sdk_uri.to_s).to eq "https://127.0.0.1/sdk"
    end

    it "delegates to URI to wrap IPv6 address in []" do
      expect(described_class.new("::1", nil, nil, nil).sdk_uri.to_s).to eq "https://[::1]/sdk"
    end
  end
end
